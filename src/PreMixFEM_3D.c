...
PetscErrorCode PC_setup(PC pc) {
  ...
  PetscCheck(s_ctx->sub_domains >= 2, PETSC_COMM_WORLD, PETSC_ERR_ARG_WRONG, "This routine is designed for multiple subdomains per process, while the input subdomain=%d.\n", s_ctx->sub_domains);
...
PetscErrorCode PC_setup_lite(PC pc) {
  ...
  PCCtx *s_ctx;
  PetscCall(PCShellGetContext(pc, &s_ctx));
  PetscCheck(s_ctx->sub_domains == 1, PETSC_COMM_WORLD, PETSC_ERR_ARG_WRONG, "This routine is designed for a single subdomain per process, while the input subdomain=%d.\n", s_ctx->sub_domains);
  ...
  *********************************************************************/
  ...
  /*********************************************************************
  Get level-3, in the lite version, no need to consider level-2.
  *********************************************************************/
  Mat A_cc;
  PetscInt A_cc_range[NEIGH + 1][2];
  const PetscMPIInt *ng_ranks;
